...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'sync') {
      ...
      let mcId;
      if (vendorProduct.mc_store_product_id) {
        await sb(`products?id=eq.${vendorProduct.mc_store_product_id}`, { method: 'PATCH', body: JSON.stringify(shared) });
        mcId = vendorProduct.mc_store_product_id;
      } else {
        const [created] = await sb('products', { method: 'POST', body: JSON.stringify(shared) });
        mcId = created.id;
    ...
    if (action === 'unsync') {
      if (vendorProduct.mc_store_product_id) {
        await sb(`products?id=eq.${vendorProduct.mc_store_product_id}`, { method: 'PATCH', body: JSON.stringify({ is_active: false }) });
      }
